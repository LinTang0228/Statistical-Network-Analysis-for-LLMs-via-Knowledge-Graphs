...
class SpectralCommunityDetection(CommunityDetectionBase):
    """Regular Spectral Clustering without SCORE (normalized Laplacian)"""
    
    def __init__(self, k: int):
        if k is None:
            raise ValueError("k must be provided for SpectralCommunityDetection")
        self.k = k
...
class NAC1CommunityDetection:
    
    def __init__(self, k: int, use_embeddings: bool = True):
        if k is None:
            raise ValueError("k must be provided for NAC1CommunityDetection")
        self.k = k
    ...
    def detect_communities(self, graph: nx.Graph, entities: List[Any] = None, 
        ...
        try:
            ...
            X = self._build_feature_matrix(graph, nodes, entities, embeddings)
    ...
    def _build_feature_matrix(self, graph: nx.Graph, nodes: List[str], 
...
class NAC2CommunityDetection:
    
    def __init__(self, k: int, alpha: float = None, adaptive_alpha: bool = True):
        if k is None:
            raise ValueError("k must be provided for NAC2CommunityDetection")
        self.k = k
        self.alpha = alpha
    ...
    def detect_communities(self, graph: nx.Graph, entities: Any = None,
        ...
        try:
            ...
            nac1 = NAC1CommunityDetection(k=effective_k)
            X = nac1._build_feature_matrix(graph, nodes, entities, embeddings)
