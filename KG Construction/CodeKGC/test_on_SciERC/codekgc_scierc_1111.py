...
parser = argparse.ArgumentParser(description="CodeKGC for SciERC")
...
def codekgc(config_path: str = args.config):
    ...
    for i, item in enumerate(data):
        ...
        text_prompt = '\n""" ' + text + ' """'
        # strict_instruction = "\nExtract triples using only the provided relations: Used-for, Feature-of, Hyponym-of, Part-of, Conjunction, Compare, Evaluate-for. Do not use or invent other relations. Include all valid triples. Use exact entity phrases from the text without adding or removing words. Do not generate redundant triples."
        # strict_instruction = "\nExtract triples using only the provided relations: Used-for, Feature-of, Hyponym-of, Part-of, Conjunction, Compare, Evaluate-for. Output 'NULL' or empty list if no no valid triples. Do not use or invent other relations. Include all valid triples but avoid redundancies. Use exact entity phrases from the text without combining, adding, or removing words—split compounds if needed."
        strict_instruction = "\nExtract triples using only the provided relations: Used-for, Feature-of, Hyponym-of, Part-of, Conjunction, Compare, Evaluate-for. Output 'NULL' or empty list if no valid triples. Do not use or invent other relations. Include all valid triples but avoid redundancies. Use exact entity phrases from the text without combining, adding, or removing words."
    ...
    if any(all_pred_strings) and any(all_gold_strings):
        ...
        # Greedy BERTScore (Previous "G-BERTScore" - Greedy Matching)
        ...
        # G-BERTScore (Hungarian Matching) from EXPLAGRAPHS paper
        ...
        macro_gbs_p = np.nanmean(precisions_h) if len(precisions_h) > 0 else 0.0
        macro_gbs_r = np.nanmean(recalls_h) if len(recalls_h) > 0 else 0.0
        macro_gbs_f1 = np.nanmean(f1s_h) if len(f1s_h) > 0 else 0.0
        ...
        micro_gbs_p = total_matched_sum / pred_total if pred_total > 0 else 0.0
        micro_gbs_r = total_matched_sum / gold_total if gold_total > 0 else 0.0
        micro_gbs_f1 = 2 * micro_gbs_p * micro_gbs_r / (micro_gbs_p + micro_gbs_r) if micro_gbs_p + micro_gbs_r > 0 else 0.0
    ...
    logging.info(f"G-BERTScore (Micro): P: {micro_gbs_p:.4f} R: {micro_gbs_r:.4f} F1: {micro_gbs_f1:.4f}")
    logging.info(f"G-BERTScore (Macro): P: {macro_gbs_p:.4f} R: {macro_gbs_r:.4f} F1: {macro_gbs_f1:.4f}")
